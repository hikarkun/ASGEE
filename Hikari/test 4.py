...
class NineChainBot:
    ...
    def login(self):
        ...
        if not self.email or not self.password or self.email.startswith("akun") or self.email.startswith("user@"):
            self.log("AUTH", "Email / Password belum dikonfigurasi di accounts4.json", "ERROR")
            return False
    ...
    def auto_upgrade_node_tier(self):
        """
        Auto Upgrade Node Tier sampai maksimum yang bisa dibeli dengan saldo.
        Akan terus upgrade Tier berikutnya selama server menyatakan affordable=True.
        Berhenti otomatis jika:
        - saldo tidak cukup,
        - sudah mencapai max tier,
        - server menolak upgrade,
        - atau terjadi error.
        """
        try:
            self.log("UPGRADE", "Mengecek Node Tier untuk auto-upgrade sampai maksimal...", "INFO")

            max_upgrades_per_run = 50  # pengaman agar tidak terjadi loop tak terbatas
            upgrade_count = 0

            while upgrade_count < max_upgrades_per_run:
                # Ambil state terbaru setiap kali sebelum upgrade.
                state_res = self.session.get(f"{API_BASE}/program/state", timeout=15)

                if state_res.status_code == 401:
                    self.log("UPGRADE", "Token kedaluwarsa saat mengecek Node Tier.", "ERROR")
                    return

                if state_res.status_code != 200:
                    try:
                        err = state_res.json().get("error", {}).get(
                            "message", f"HTTP {state_res.status_code}"
                        )
                    except Exception:
                        err = f"HTTP {state_res.status_code}"
                    self.log("UPGRADE", f"Gagal mengambil state: {err}", "WARN")
                    return

                raw = state_res.json()
                data = raw.get("data", raw) if isinstance(raw, dict) else {}

                current_tier = int(data.get("nodeTier", self.stats.get("tier", 1)))
                current_xp = float(data.get("xpTotal", self.stats.get("xp_total", "0")) or 0)

                self.stats["tier"] = current_tier
                self.stats["xp_total"] = str(current_xp)

                next_tier_data = data.get("nextTier")
                if not isinstance(next_tier_data, dict):
                    next_tier_data = {}

                target_tier = next_tier_data.get("tier")
                cost_value = next_tier_data.get("cost")
                affordable = next_tier_data.get("affordable")

                # Jika server tidak menyediakan nextTier, kemungkinan sudah max tier.
                if target_tier is None:
                    max_tier = data.get("maxNodeTier")
                    if max_tier is not None:
                        self.log(
                            "UPGRADE",
                            f"Current Tier: {current_tier} │ Max Tier: {max_tier} │ "
                            f"Saldo: {format_num(current_xp)} LOVE9",
                            "INFO"
                        )
                        if current_tier >= int(max_tier):
                            self.log("UPGRADE", "Sudah mencapai MAX Node Tier.", "SUCCESS")
                        else:
                            self.log(
                                "UPGRADE",
                                "Server tidak memberikan informasi nextTier.",
                                "WARN"
                            )
                    else:
                        self.log(
                            "UPGRADE",
                            "Server tidak memberikan nextTier. Upgrade dihentikan.",
                            "WARN"
                        )
                    return
                ...
                try:
                    ...
                    self.log("UPGRADE", f"Target Tier tidak valid: {target_tier!r}", "WARN")
                    return

                if cost_value is None:
                    self.log(
                        "UPGRADE",
                        f"Tier {current_tier} → {target_tier}: server tidak memberikan biaya.",
                        "WARN"
                    )
                    return
                ...
                try:
                    ...
                    self.log("UPGRADE", f"Biaya Tier tidak valid: {cost_value!r}", "WARN")
                    return

                self.log(
                    "UPGRADE",
                    f"Current Tier: {current_tier} │ Saldo: {format_num(current_xp)} LOVE9 │ "
                    f"Target: Tier {target_tier} │ Cost: {format_num(cost)} LOVE9",
                    "INFO"
                )

                # Jangan upgrade jika server menyatakan tidak affordable
                # atau saldo memang belum cukup.
                if affordable is False or current_xp < cost:
                    kurang = max(0, cost - current_xp)
                    self.log(
                        "UPGRADE",
                        f"Berhenti di Tier {current_tier}. "
                        f"Tier {target_tier} membutuhkan {format_num(cost)} LOVE9"
                        + (
                            f" │ Kurang {format_num(kurang)} LOVE9."
                            if kurang > 0 else ""
                        ),
                        "WARN"
                    )
                ...
                self.log(
                    "UPGRADE",
                    f"Saldo cukup! Upgrade Node Tier {current_tier} → {target_tier}...",
                    "INFO"
                )

                up_res = self.session.post(
                    f"{API_BASE}/program/node/upgrade",
                    json={"toTier": target_tier},
                    timeout=15
                )
                ...
                if up_res.status_code not in [200, 201] or not isinstance(up_body, dict):
                    if isinstance(up_body, dict):
                        err_obj = up_body.get("error", {})
                        if isinstance(err_obj, dict):
                            err = err_obj.get("message") or err_obj.get("code")
                        else:
                            err = err_obj
                        err = err or up_body.get("message") or "Upgrade gagal"
                    else:
                        err = up_res.text[:1000] or f"HTTP {up_res.status_code}"

                    self.log(
                        "UPGRADE",
                        f"Upgrade Tier {target_tier} gagal │ HTTP {up_res.status_code} │ {err}",
                        "WARN"
                    )
                    return

                success = up_body.get("success", False)
                result = up_body.get("data", {}) or {}

                if not success:
                    self.log(
                        "UPGRADE",
                        f"Server menolak upgrade Tier {target_tier}: "
                        f"{json.dumps(up_body, ensure_ascii=False)}",
                        "WARN"
                    )
                    return

                tier_before = result.get("tierBefore", current_tier)
                tier_after = result.get("tierAfter", target_tier)
                actual_cost = result.get("cost", cost)
                new_state = result.get("state", {}) or {}

                # Update state dari response server.
                self.stats["tier"] = int(new_state.get("nodeTier", tier_after))
                self.stats["xp_total"] = str(
                    new_state.get("xpTotal", max(0, current_xp - float(actual_cost)))
                )

                if "contributionRate" in new_state:
                    self.stats["contribution_rate"] = str(new_state["contributionRate"])
                if "tapsRemaining" in new_state:
                    self.stats["taps_remaining"] = int(new_state["tapsRemaining"])

                upgrade_count += 1

                self.stats.setdefault("upgrades", []).append(
                    f"Node Tier {self.stats['tier']}"
                )

                self.log(
                    "UPGRADE",
                    f"SUCCESS! Node Tier {tier_before} → {self.stats['tier']} "
                    f"│ Cost: {format_num(actual_cost)} LOVE9",
                    "SUCCESS"
                )

                # Jika response upgrade sudah memberikan nextTier,
                # tampilkan informasinya sebelum loop berikutnya.
                server_next = new_state.get("nextTier")
                if isinstance(server_next, dict):
                    next_num = server_next.get("tier")
                    next_cost = server_next.get("cost")
                    next_affordable = server_next.get("affordable")

                    if next_num is not None and next_cost is not None:
                        status = (
                            "BISA UPGRADE"
                            if next_affordable is True
                            else "belum cukup"
                            if next_affordable is False
                            else "akan dicek ulang"
                        )
                        self.log(
                            "UPGRADE",
                            f"Next Tier: {next_num} │ Cost: {format_num(next_cost)} LOVE9 │ {status}",
                            "INFO"
                        )

                # Loop akan mengambil state terbaru lagi dan mencoba Tier berikutnya.
                # Tidak perlu sleep karena endpoint server langsung mengembalikan state terbaru.

            self.log(
                "UPGRADE",
                f"Pengaman loop tercapai ({max_upgrades_per_run} upgrade dalam satu run).",
                "WARN"
            )

        except Exception as e:
            self.log("UPGRADE", f"Error auto upgrade Node Tier: {e}", "WARN")
...
def run_all_accounts(config):
    ...
    for idx, acc in enumerate(accounts, start=1):
        ...
        if state:
            ...
            # Components hanya dijalankan jika secara eksplisit diaktifkan.
            # Default = False agar bot fokus pada upgrade Node Tier.
            if settings.get("auto_upgrade_components", False):
                bot.auto_upgrade_components()

            # 5. Auto Upgrade Node Tier (Tier 1 -> Tier 2 -> Tier 3)
            # Default = True agar Node Tier tetap menjadi prioritas utama.
...
def main():
    print(BANNER)
    config_path = os.path.join(os.path.dirname(__file__), "accounts4.json")
    ...
    if not os.path.exists(config_path):
        if os.path.exists(example_path):
            ...
            print(f"  {C_WARN}[!] File accounts4.json otomatis dibuat dari template accounts.example.json.{RESET}")
            print(f"  {C_MUTED}[*] Silakan edit file accounts4.json dan masukkan akun Anda:{RESET}")
            print(f"      {BOLD}nano accounts4.json{RESET}\n")
        else:
    ...
    try:
        while True:
            ...
            try:
                ...
                print(f"  {C_ERROR}✗ Gagal membaca accounts4.json: {e}{RESET}")
                time.sleep(10)
                continue
