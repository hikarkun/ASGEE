...
class NineChainBot:
    ...
    def login(self):
        ...
        if not self.email or not self.password or self.email.startswith("akun") or self.email.startswith("user@"):
            self.log("AUTH", "Email / Password belum dikonfigurasi di accounts5.json", "ERROR")
            return False
...
def main():
    print(BANNER)
    config_path = os.path.join(os.path.dirname(__file__), "accounts5.json")
    ...
    if not os.path.exists(config_path):
        if os.path.exists(example_path):
            ...
            print(f"  {C_WARN}[!] File accounts5.json otomatis dibuat dari template accounts.example.json.{RESET}")
            print(f"  {C_MUTED}[*] Silakan edit file accounts5.json dan masukkan akun Anda:{RESET}")
            print(f"      {BOLD}nano accounts5.json{RESET}\n")
        else:
    ...
    try:
        while True:
            ...
            try:
                ...
                print(f"  {C_ERROR}✗ Gagal membaca accounts5.json: {e}{RESET}")
                time.sleep(10)
                continue
